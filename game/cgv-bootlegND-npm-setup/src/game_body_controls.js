  ...
  async init() {
    ...
    const createPlatform = (length, width, tileColorMap) => {
      ...
      for (let x = 0; x < length; x++) {
        ...
        for (let z = 0; z < width; z++) {
          ...
          newTile.applyMatrix4(
            new THREE.Matrix4().makeTranslation(xpos, y, zpos)
          );
      ...
      geometriesTiles.applyMatrix4(
        new THREE.Matrix4().makeTranslation(-length / 2, 0, -width / 2)
      );
      geometriesTiles.applyMatrix4(
...
function followShip() {
  ...
  if (keys.w) shipSpeed = 10;
  else if (keys.s) shipSpeed = -10;
